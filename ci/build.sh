...
docker buildx build --pull --push   --build-context "kernel=$workspace_root/kernel-go"   --build-context "protocol=$workspace_root/liveshop-protocol/platform"   --file "$CI_PROJECT_DIR/business/backend/Dockerfile"   --tag "$CI_REGISTRY_IMAGE/backend:$tag"   "$CI_PROJECT_DIR/business/backend"
